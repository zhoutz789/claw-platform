...
> S0 状态：**工程脚手架已就绪（2026-08-20）**；S1 状态：**用户/权限/KYC + 角色包引擎 + 资产域已落地（2026-08-20）**；全球化框架：**法域主表 + 适配器注册表 + 国家清单接口已落地（2026-08-20）**；S2 状态：**复式记账引擎 + 三专户 + 押金流转 + 站点现货 + DTI 合规已落地（2026-08-20）**；S3 状态：**换电域（下单/押金双向流转/预扣结算/地图适配层）已落地（2026-08-20）**；S4 状态：**服务站 APP（扫码收发/电池位/日账单）+ ABA 托管对接（三专户 KHQR 收单 + Bakong 清算、充值/提现/T+1 对账）已落地（2026-08-20）**
